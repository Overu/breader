 ...
 * 
 * Copyright (c) 2011, Red Hat Inc. or third-party contributors as indicated by the @author tags or
...
public class BeanValidationIntegrator implements Integrator {
  ...
  private static enum ValidationMode {
    AUTO, CALLBACK, NONE, DDL;

    public static Set<ValidationMode> getModes(final Object modeProperty) {
    ...
    private static ValidationMode getMode(final String modeProperty) {
      if (modeProperty == null || modeProperty.length() == 0) {
        ...
        try {
          ...
          throw new HibernateException("Unknown validation mode in " + MODE_PROPERTY + ": "
              + modeProperty);
        }
      }
    }
  ...
  private static final String ACTIVATOR_CLASS =
  ...
  public static void validateFactory(final Object object) {
    try {
      final Class activatorClass =
          BeanValidationIntegrator.class.getClassLoader().loadClass(ACTIVATOR_CLASS);
      try {
        ...
        throw new HibernateException(
            "Could not locate method needed for ValidatorFactory validation", e);
      }
    } catch (HibernateException e) {
      throw e;
    } catch (Exception e) {
  ...
  public void integrate(final Configuration configuration,
    ...
    final Set<ValidationMode> modes =
        ValidationMode.getModes(configuration.getProperties().get(MODE_PROPERTY));
    ...
    // todo : if this works out, probably better to simply alter TypeSafeActivator into a single
    // method...
    applyRelationalConstraints(modes, isBeanValidationAvailable, typeSafeActivatorClass,
        configuration, dialect

    );
    applyHibernateListeners(modes, isBeanValidationAvailable, typeSafeActivatorClass,
        configuration, sessionFactory, serviceRegistry);
  }
  ...
  public void integrate(final MetadataImplementor metadata,
    ...
    // final ClassLoaderService classLoaderService = serviceRegistry.getService(
    ...
    // // todo : if this works out, probably better to simply alter TypeSafeActivator into a single
    // method...
    // applyRelationalConstraints(modes, isBeanValidationAvailable, typeSafeActivatorClass, props,
    // metadata);
    // applyHibernateListeners(modes, isBeanValidationAvailable, typeSafeActivatorClass,
  ...
  // throw new HibernateException("Bean Validation not available in the class path but required in "
  // + MODE_PROPERTY);
  // if(modes.contains(ValidationMode.AUTO)) return; //nothing to activate
  // }
  // try {
  // Method applyDDLMethod = typeSafeActivatorClass.getMethod(DDL_METHOD, Iterable.class,
  // Properties.class, ClassLoaderService.class);
  // try {
  ...
  private void applyHibernateListeners(final Set<ValidationMode> modes,
      final boolean beanValidationAvailable, final Class typeSafeActivatorClass,
      final Configuration configuration, final SessionFactoryImplementor sessionFactory,
      final SessionFactoryServiceRegistry serviceRegistry) {
    // de-activate not-null tracking at the core level when Bean Validation is present unless the
    // user explicitly
    // asks for it
    ...
    if (!beanValidationAvailable) {
      if (modes.contains(ValidationMode.CALLBACK)) {
        throw new HibernateException(
            "Bean Validation not available in the class path but required in " + MODE_PROPERTY);
    ...
    try {
      Method activateMethod =
          typeSafeActivatorClass.getMethod(ACTIVATE_METHOD, EventListenerRegistry.class,
              Configuration.class);
      try {
  ...
  // }
  // try {
  // Method activateMethod = typeSafeActivatorClass.getMethod(ACTIVATE_METHOD,
  ...
  private void applyRelationalConstraints(final Set<ValidationMode> modes,
      final boolean beanValidationAvailable, final Class typeSafeActivatorClass,
      final Configuration configuration, final Dialect dialect) {
    ...
    try {
      Method applyDDLMethod =
          typeSafeActivatorClass.getMethod(DDL_METHOD, Collection.class, Properties.class,
              Dialect.class);
      try {
        applyDDLMethod.setAccessible(true);
        applyDDLMethod.invoke(null, configuration.createMappings().getClasses().values(),
            configuration.getProperties(), dialect);
      } catch (HibernateException e) {
        throw e;
      } catch (Exception e) {
        throw new HibernateException("Error applying BeanValidation relational constraints", e);
  ...
  private Class loadTypeSafeActivatorClass(final SessionFactoryServiceRegistry serviceRegistry) {
    try {
