...
public final class LabsGinModule extends AbstractGinModule {
  ...
  public static class BinderProvider implements Provider<Binder> {
    ...
    public Binder get() {
      ...
      isWidgetMapBinder.bind("/").toAsyncProvider(labs);
      isWidgetMapBinder.bind(WaveTest.class.getName()).toAsyncProvider(waveTest);
      isWidgetMapBinder.bind(BookFlip.class.getName()).toAsyncProvider(bookFlip);
      ...
      isWidgetMapBinder.bind(NestedBlipTest.class.getName()).toAsyncProvider(nestedBlipTest);
      isWidgetMapBinder.bind(LibraryView.class.getName()).toAsyncProvider(libraryView);
      isWidgetMapBinder.bind(TestEditor.class.getName()).toAsyncProvider(testEditor);
      isWidgetMapBinder.bind(SearchPanel.class.getName()).toAsyncProvider(searchPanel);
      isWidgetMapBinder.bind(BlipTest.class.getName()).toAsyncProvider(blipTest);
      isWidgetMapBinder.bind(ContactPanel.class.getName()).toAsyncProvider(contactPanel);
      isWidgetMapBinder.bind(TreeTest.class.getName()).toAsyncProvider(treeTest);
